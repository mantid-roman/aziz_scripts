...
class files:
	...
	def initialize(self,Cycle,user,prefFile="", prefDir="", Verbose=False):
		...
		self.SampleNumberOfWavelengthPoints  =   self.updatePrefVal("SampleNumberOfWavelengthPoints",self.SampleNumberOfWavelengthPoints)
	...
	def VanCorrFileLookup(self):
		...
		if self.ExistV == "yes" or self.ExistV =="sam":
			if VanOK <> 0:
				msg = msg + "At least the corrected vanadium file "+self.CorrVanFile+"-"+str(i)+".nxs is  missing \n"
				if self.ExistV =="sam" :
			...
			if self.ExistV == "over":
				...
				msg = msg +  "At least the corrected Van file "+self.CorrVanFile+"-"+str(i)+".nxs given in the pref file already exists,\n"
	...
	def updateCalib(self):
		...
		self.VEmptyFile  = CRY_utils.ListOfList2List(CRY_utils.getSampleList(self.basefile,self.VErunnoList,self.VEmptyDir))
		self.SEmptyFile  = CRY_utils.ListOfList2List(CRY_utils.getSampleList(self.basefile,self.SErunnoList,self.SEmptyDir))
