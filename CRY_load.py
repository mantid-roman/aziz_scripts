...
def SplitBank(InputArea,bankList, Del=True):
	for i in bankList:
		CropWorkspace(InputWorkspace=InputArea, OutputWorkspace=InputArea+"-"+str(i), StartWorkspaceIndex=i-1, EndWorkspaceIndex=i-1)	
	if Del:
...
def setsDrange(wkspc,expt):
	...
	for i in range(0,expt.Nbank):
		...
		if datbin>float(expt.Bining[i]):
			print 'WARNING: Rebining in *pref file '+expt.Bining[i]+' is lower than diffraction focusing rebining step'
