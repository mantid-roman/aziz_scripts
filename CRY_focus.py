...
# Loads SAC/Efficiency correction in wkspc "Corr" or sets it to "1"
	...
	if NoSAC:
		...
		if uampstotal<1e-6:
			...
			MaskDetectorsIf(InputWorkspace="VanadiumSum",InputCalFile=expt.Path2GrpFile,OutputCalFile=newCalFile,Mode="DeselectIf",Operator="LessEqual",Value=10)	
			if not expt.debugMode:
...
def Focus(expt, sampleAdd, scale, Norm, isfirst=False, NoAbs=False):
	...
	MaskDetectorsIf(InputWorkspace="sampleSum",InputCalFile=expt.Path2GrpFile,OutputCalFile=newCalFile,Mode="DeselectIf",Operator="Equal",Value=10)	
	...
	DiffractionFocussing(InputWorkspace="sample",OutputWorkspace="sample",GroupingFileName=expt.Path2DatGrpFile, PreserveEvents=False)	
...
def DivideSampVana(expt,Norm):	
	...
	if Norm:
		...
		for i in expt.bankList:
			RebinToWorkspace(WorkspaceToRebin="Vanadium-"+str(i),WorkspaceToMatch="sample-"+str(i),OutputWorkspace="Vanadium-"+str(i))
