...
def CreateVana(expt, NoAbs=False):
	...
	DiffractionFocussing(InputWorkspace="Vanadium_corr",OutputWorkspace="Vanadium_foc",GroupingFileName=GrpFile, PreserveEvents=False)
...
def Removebins(expt):
	for i in expt.bankList:
		spec=i-1
		SmoothData(InputWorkspace="Vanadium-"+str(i), OutputWorkspace="Vanadium-"+str(i), NPoints=int(expt.VanSmooth))
